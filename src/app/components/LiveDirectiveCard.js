...
export const LiveDirectiveCard = ({ task, onToggle, onDelete }) => {
  ...
  return (
    <div
      className={`group flex justify-between items-center p-3 rounded-sm border transition-all ${
        task.completed
    ...
    >
      ...
      <div className="flex items-center gap-3 overflow-hidden flex-1">
        ...
        <div
          ...
          className={`w-5 h-5 shrink-0 border-2 flex items-center justify-center transition-colors rounded-sm cursor-pointer ${
            task.completed
      ...
      {/* --- RIGHT SIDE: TACTICAL PURGE BUTTON --- */}
      {(task.completed || isExpired) && (
        <button
          onClick={() => onDelete(task.id)}
          className="ml-3 p-2 text-gray-400 hover:text-red-500 hover:bg-red-500/10 rounded-sm transition-all cursor-pointer shrink-0"
